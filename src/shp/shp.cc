...
bool Shp::ConvertAreaToImage(const cv::Rect_<double> &area_geo_bound,
    ...
    // 确定地图内的边界
    cv::Rect2d vaild_geo_bound = area_geo_bound;
    if (area_geo_bound.x<geo_bound_.x)
        vaild_geo_bound.x = geo_bound_.x;
    if(area_geo_bound.x+area_geo_bound.width>geo_bound_.x+geo_bound_.width)
        vaild_geo_bound.width = geo_bound_.x + geo_bound_.width - vaild_geo_bound.x;
    if(area_geo_bound.y<geo_bound_.y)
        vaild_geo_bound.y = geo_bound_.y;
    if(area_geo_bound.y+area_geo_bound.height>geo_bound_.y+geo_bound_.height)
        vaild_geo_bound.height = geo_bound_.y + geo_bound_.height - vaild_geo_bound.y;
...
bool Shp::ShowOnMap(const cv::Mat &show_img,
    ...
    if(black_background){
        ConvertAreaToImage(cv::Rect2d(min_x, min_y, max_x-min_x, max_y-min_y),1, result_show_img, 5, color, black_background);
        src_weight = 0.8;
    }
    else{
        ConvertAreaToImage(cv::Rect2d(min_x, min_y, max_x-min_x, max_y-min_y),1, result_show_img, 5, color, black_background);
